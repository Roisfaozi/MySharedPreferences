package com.raven.mysharedpreferences
...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode== REQUEST_CODE) {
            if (requestCode== FormUserPreferenceActivity.RESULT_CODE) {
                userModel = data?.getParcelableExtra(FormUserPreferenceActivity.EXTRA_TYPE_FORM) as UserModel
    ...
    private fun populateView(userModel: UserModel){
        ...
        tv_email.text = if (userModel.name.toString().isEmpty()) "Tidak ada" else userModel.email
